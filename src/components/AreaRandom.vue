<template>
    <div>
        ...
        <group title="城市列表">
            <cell v-for="city in cityList[areaSelected]" :title="city+'('+ nodeNum[city].cur+'/'+nodeNum[city].total+')'" @click.native="showList(city)">
                <x-button mini @click.native.stop="addNode(city)">+</x-button>
            </cell>
        </group>
        <group :title="citySelected+'节点列表'" v-if="showScroll">
            <x-button mini style="position: absolute;right: 17px;top: -10px;z-index:99" @click.native="showScroll=false;citySelected=''">x</x-button>
            <scroller lock-x scrollbar-y style="max-height: 130px">
                <div>
                    <cell style="padding-left: 30px" v-for="node in nodeList[citySelected]" :title="node.name+',延迟:'+node.delay+'ms,状态:'+node.state"><span class="dot" :style="{background:node.color}"></span>
                    </cell>
                </div>
            </scroller>
        </group>
...
<script>
    ...
    export default {
        ...
        props: ['permission', 'areaList', 'cityList', 'nodeList', 'inOutData', 'user'],
        methods: {
            addNode(city){
