<template>
    <div>
        ...
        <div :is="curWay" :permission="permission" :areaList="areaList" :cityList="cityList" :nodeList="nodeList" :areaNodeList="areaNodeList" :inOutData="inOutData"></div>
    </div>
</template>
    ...
    export default {
        ...
        beforeMount(){
            let index = location.href.indexOf('?user=') + 6,
                _this = this;
                window.user = location.href.substring(index);
            console.log(user);
            this.ajax({
                url: '/getServers',
                data: user,
